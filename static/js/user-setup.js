...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function handleProfileCompletion() {
    ...
    completeBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i>Creating Profile...';
    
    // Create user profile via API
    const profile = {
      username: formData.fullName.replace(/\s+/g, '_').toLowerCase() + '_' + Date.now(),
      email: `${formData.fullName.replace(/\s+/g, '_').toLowerCase()}@temp.com`, // Temp email
      personal_info: {
        full_name: formData.fullName,
        age: formData.age,
        birthday: formData.birthday
      },
      allergens: [...formData.foodAllergens, ...formData.beautyAllergens],
      preferences: [...formData.dietPreferences, ...formData.healthPreferences],
    ...
    .then(data => {
      if (data.success) {
        // Save user_id to localStorage for later use
        localStorage.setItem('rekada_user_id', data.user_id);
        localStorage.setItem('rekada_user_profile', JSON.stringify({
          ...formData,
